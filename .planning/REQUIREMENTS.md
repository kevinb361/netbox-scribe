...
- [ ] **REQ-002** — the client retrieves every page of synthetic NetBox device results and reports authentication, transport, and malformed-response failures without exposing the API token (milestone: v0.1) (depends: REQ-001)
- [ ] **REQ-003** — `nbscribe export` normalizes device records into schema-versioned canonical YAML with stable ordering and byte-identical output for unchanged input (milestone: v0.1) (depends: REQ-002)
